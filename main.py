...
stocks = input('Enter up to 5 ticker symbols, separated by commas: ').upper()
stocks = stocks.split(", ")
stocks = ['$' + x for x in stocks]  # Add cashtag symbol

#                                  -------------------- Twitter Analysis --------------------
def twitter_sentiment(stocks):
            ...
            if not holiday_df['Date'].eq(tweet_date).any():
                if tweets_per_day < 100:
                    if current_day is None:
                        current_day = day
                    if day.date() != current_day.date():  # Only append tweets from the most recent trading day
                        index += 1
                        tweets_per_day = 0
                        break
                    ...
                    if index >= len(stocks):
                        break
                    search_parameters['q'] = stocks[index]  # Update the search query to the next stock
                    cursor = tweepy.Cursor(api.search_tweets,
                                           **search_parameters)  # Update the cursor with the new search parameters
                    tweet_iterator = cursor.items(limit=1000)  # Reset the tweet iterator
                    tweets_per_day = 0  # Reset the tweets_per_day counter

    # Create a df to store tweets
